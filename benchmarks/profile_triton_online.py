"""
Triton Online Softmax Profiling
================================
使用 PyTorch Profiler 对 Online Softmax（两遍扫描）Triton kernel 进行性能分析。
与三遍扫描版本对比，验证减少一次全局内存加载带来的性能收益。

输出：
  1. 按 CUDA 时间排序的控制台表格。
  2. Chrome trace JSON → traces/triton_online_trace.json

三遍 vs Online 两遍对比：
  - 三遍：load→max / load→exp+sum / load→normalize    共 3×T 次全局内存读取
  - 两遍：load→online_max+sum / load→normalize         共 2×T 次全局内存读取（-33%）

用法：
    python benchmarks/profile_triton_online.py [--seq_len 128] [--hidden_size 768] \
...
from models.triton_attention import OnlineTritonAttention


class OnlineTritonTransformerBlock(MiniTransformerBlock):
    """
    使用 Online Softmax Triton 融合 attention 的 TransformerBlock。

    继承 MiniTransformerBlock，仅将 attn_fn 替换为 OnlineTritonAttention。
    """

    def __init__(self, config=None):
        super().__init__(config, use_sdpa=False)
        self.attn_fn = OnlineTritonAttention(config or TransformerConfig())


def parse_args():
    p = argparse.ArgumentParser(description="Triton Online Softmax 性能分析")
...
def main():
    ...
    print("=" * 60)
    print("  Triton Online Softmax Profiling（两遍扫描）")
    print("=" * 60)
    ...
    print(f"     scale+mask+softmax    → Triton Online kernel（两遍扫描）")
    print(f"     PV matmul             → cublas（保留）")
    print(f"\n  📊 与三遍版本的区别：")
    print(f"     三遍: 3×T 全局内存读取（max / exp+sum / normalize）")
    print(f"     两遍: 2×T 全局内存读取（online_max+sum / normalize）→ 减少 33%")

    # ---- 模型 ----
    model = OnlineTritonTransformerBlock(cfg).to(cfg.device).to(cfg.dtype)
    model.eval()
    ...
    trace_path = trace_dir / "triton_online_trace.json"
    ...
    with profile(
        ...
        with torch.no_grad():
            for _ in range(args.repeat):
                with record_function("transformer_block_triton_online"):
                    _ = model(x)
        torch.cuda.synchronize()
    ...
    # ---- Kernel 统计摘要 ----
    print("\n" + "=" * 60)
    print("  📊 Kernel 统计摘要（Online Softmax）")
    print("=" * 60)
    ...
    triton_events = [e for e in cuda_events if "triton" in e.key.lower()
                     or "online_softmax" in e.key.lower()
    ...
    print("  📊 与其他版本的预期对比")
    print("=" * 60)
    print("  baseline:       scale / mask / softmax 各 1 个 kernel")
    print("  triton (三遍):  scale+mask+softmax 融合, 3×T 全局 load")
    print("  triton (两遍):  scale+mask+softmax 融合, 2×T 全局 load ← 当前")
    ...
    print("   python benchmarks/profile_triton.py          → triton_trace.json")
    print("   python benchmarks/profile_triton_online.py   → triton_online_trace.json  ← 当前")
